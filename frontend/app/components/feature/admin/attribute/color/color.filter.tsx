...
interface ColorFilterProps {
  ...
  onSortByChange: (sortBy: SortColorBy) => void;
...
export const ColorFilter = ({
  ...
  const hasActiveFilters =
    search || sortBy !== SortColorBy.NAME || sortOrder !== SortOrder.ASC;
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      {isExpanded && (
        <div className="flex flex-col sm:flex-row gap-4 p-4 rounded-lg border bg-muted/30 animate-in slide-in-from-top-2 duration-200">
          {/* Sort by filter */}
          <div className="flex-1 space-y-2">
            <label className="text-sm font-medium">Sắp xếp theo</label>
            <Select
              value={sortBy}
              onValueChange={(value) => onSortByChange(value as SortColorBy)}
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Chọn cột sắp xếp" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={SortColorBy.NAME}>Tên màu</SelectItem>
                <SelectItem value={SortColorBy.HEX}>Mã màu</SelectItem>
                <SelectItem value={SortColorBy.CREATED_AT}>Ngày tạo</SelectItem>
              </SelectContent>
            </Select>
          </div>
