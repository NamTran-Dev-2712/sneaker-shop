import { useState, useCallback } from "react";
import { Plus } from "lucide-react";
import { Button } from "~/components/ui/button";
import { DeleteModal } from "~/components/common/modal/modal.delete";
import { Pagination } from "~/components/common/shared/pagination";
import { SizeFilter } from "./size.filter";
import { SizeListItem } from "./size.list-item";
import { SizeModal } from "./size.modal";
import { SizeQuickStatistic } from "./size.quick-statistic";
import {
  useSizeList,
  useCreateSize,
  useUpdateSize,
  useDeleteSize,
} from "~/hooks/react-query/use-size.query";
import type { GetSizeItem } from "~/services/attribute/size/dto/get-size/get-size.response";
import type { GetSizeRequest } from "~/services/attribute/size/dto/get-size/get-size.request";
import type { CreateSizeFormData } from "~/lib/validation/admin/attribute/size.schema";
import { SortSizeBy } from "~/types/global/filter/sort-by.filter";
import { SortOrder } from "~/types/global/filter/sort-order.filter";

export const SizeIndex = () => {
  // Filter state
  const [query, setQuery] = useState<GetSizeRequest>({
    pageNumber: 1,
    pageSize: 10,
    search: "",
    system: undefined,
    sortBy: SortSizeBy.VALUE,
    sortOrder: SortOrder.ASC,
  });

  // Modal states
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [selectedSize, setSelectedSize] = useState<GetSizeItem | null>(null);

  // React Query hooks
  const { data, isLoading, error } = useSizeList(query);
  const createMutation = useCreateSize();
  const updateMutation = useUpdateSize();
  const deleteMutation = useDeleteSize();

  // Handlers
  const handleSearchChange = useCallback((search: string) => {
    setQuery((prev) => ({ ...prev, search, pageNumber: 1 }));
  }, []);

  const handleSystemChange = useCallback((system: string | undefined) => {
    setQuery((prev) => ({ ...prev, system, pageNumber: 1 }));
  }, []);

  const handleSortByChange = useCallback((sortBy: SortSizeBy) => {
    setQuery((prev) => ({ ...prev, sortBy }));
  }, []);

  const handleSortOrderChange = useCallback((sortOrder: SortOrder) => {
    setQuery((prev) => ({ ...prev, sortOrder }));
  }, []);

  const handleResetFilter = useCallback(() => {
    setQuery({
      pageNumber: 1,
      pageSize: 10,
      search: "",
      system: undefined,
      sortBy: SortSizeBy.VALUE,
      sortOrder: SortOrder.ASC,
    });
  }, []);

  const handlePageChange = useCallback((page: number) => {
    setQuery((prev) => ({ ...prev, pageNumber: page }));
  }, []);

  const handlePageSizeChange = useCallback((pageSize: number) => {
    setQuery((prev) => ({ ...prev, pageSize, pageNumber: 1 }));
  }, []);

  const handleAdd = useCallback(() => {
    setSelectedSize(null);
    setIsModalOpen(true);
  }, []);

  const handleEdit = useCallback((size: GetSizeItem) => {
    setSelectedSize(size);
    setIsModalOpen(true);
  }, []);

  const handleDelete = useCallback((size: GetSizeItem) => {
    setSelectedSize(size);
    setIsDeleteModalOpen(true);
  }, []);

  const handleModalSubmit = useCallback(
    async (formData: CreateSizeFormData) => {
      if (selectedSize) {
        await updateMutation.mutateAsync({
          id: selectedSize.id,
          data: formData,
        });
      } else {
        await createMutation.mutateAsync(formData);
      }
      setIsModalOpen(false);
      setSelectedSize(null);
    },
    [selectedSize, createMutation, updateMutation],
  );

  const handleConfirmDelete = useCallback(async () => {
    if (selectedSize) {
      await deleteMutation.mutateAsync(selectedSize.id);
      setIsDeleteModalOpen(false);
      setSelectedSize(null);
    }
  }, [selectedSize, deleteMutation]);

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Quản lý size</h1>
          <p className="text-muted-foreground">
            Quản lý danh sách các size cho sản phẩm
          </p>
        </div>
        <Button onClick={handleAdd} className="shrink-0">
          <Plus className="h-4 w-4 mr-2" />
          Thêm size
        </Button>
      </div>

      {/* Quick Statistics */}
      <SizeQuickStatistic data={data} isLoading={isLoading} />

      {/* Filter */}
      <SizeFilter
        search={query.search || ""}
        system={query.system}
        sortBy={query.sortBy || SortSizeBy.VALUE}
        sortOrder={query.sortOrder || SortOrder.ASC}
        onSearchChange={handleSearchChange}
        onSystemChange={handleSystemChange}
        onSortByChange={handleSortByChange}
        onSortOrderChange={handleSortOrderChange}
        onReset={handleResetFilter}
      />

      {/* Error state */}
      {error && (
        <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-4 text-destructive">
          <p className="text-sm font-medium">Lỗi khi tải dữ liệu</p>
          <p className="text-sm">{error.message}</p>
        </div>
      )}

      {/* List */}
      <SizeListItem
        sizes={data?.items}
        isLoading={isLoading}
        onEdit={handleEdit}
        onDelete={handleDelete}
        onAdd={handleAdd}
      />

      {/* Pagination */}
      {data && data.totalPages > 0 && (
        <Pagination
          currentPage={query.pageNumber || 1}
          totalPages={data.totalPages}
          totalItems={data.totalItems}
          pageSize={query.pageSize || 10}
          hasPreviousPage={data.hasPreviousPage}
          hasNextPage={data.hasNextPage}
          onPageChange={handlePageChange}
          onPageSizeChange={handlePageSizeChange}
        />
      )}

      {/* Create/Edit Modal */}
      <SizeModal
        open={isModalOpen}
        onOpenChange={setIsModalOpen}
        size={selectedSize}
        onSubmit={handleModalSubmit}
        isLoading={createMutation.isPending || updateMutation.isPending}
      />

      {/* Delete Modal */}
      <DeleteModal
        open={isDeleteModalOpen}
        onOpenChange={setIsDeleteModalOpen}
        itemName={
          selectedSize
            ? `${selectedSize.system} ${selectedSize.value}`
            : undefined
        }
        onConfirm={handleConfirmDelete}
        isLoading={deleteMutation.isPending}
      />
    </div>
  );
};

export default SizeIndex;
